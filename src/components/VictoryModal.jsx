  ...
  useEffect(() => {
    if (result && !fired.current) {
      ...
      if (result.type === 'checkmate' || result.type === 'time') {
        ...
        (function frame() {
          confetti({ particleCount: 4, angle: 60, spread: 65, origin: { x: 0 }, colors: ['#c9a227', '#e0bb52', '#e8e6e1'] });
          confetti({ particleCount: 4, angle: 120, spread: 65, origin: { x: 1 }, colors: ['#c9a227', '#e0bb52', '#e8e6e1'] });
          if (Date.now() < end) requestAnimationFrame(frame);
        })();
      }
    }
    if (!result) fired.current = false;
  }, [result]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-md">
      <div className="relative w-full max-w-md rounded-3xl border border-brass-500/30 bg-ink-800/80 p-8 text-center shadow-2xl shadow-black/60 backdrop-blur-xl animate-floatIn">
        <button onClick={onClose} className="absolute right-4 top-4 text-muted hover:text-parchment">
          <X size={18} />
        </button>
        ...
        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full border border-brass-500/40 bg-brass-500/10">
          {isDraw ? <Handshake size={28} className="text-brass-400" /> : <Crown size={28} className="text-brass-400" />}
        </div>
        ...
        <button
          onClick={onNewGame}
          className="mt-6 inline-flex items-center gap-2 rounded-xl bg-brass-500 px-5 py-2.5 font-medium text-ink-950 transition hover:bg-brass-400"
        >
          <RotateCcw size={16} />
          Start a new game
        </button>
